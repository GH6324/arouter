...
- **加密策略**：在控制器全局设置里配置 `encryption_policies`（可通过 `/api/settings` 或前端“全局设置”录入），示例：
  ```json
  [
    {"id":1,"name":"aes128","method":"aes-128-gcm","key":"base64-or-hex-key"},
    {"id":2,"name":"chacha","method":"chacha20-poly1305","key":"base64-or-hex-key"}
  ]
  ```
  节点拉取配置后按“当前时间秒 % 策略数量”选用一条策略，入口加密、出口按 `enc_id` 解密，中间节点仅透传。密钥支持 Base64/HEX，方法支持 `aes-128-gcm`/`aes-256-gcm`/`chacha20-poly1305`。
